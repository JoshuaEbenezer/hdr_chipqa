...
def find_kurtosis_slice(Y3d_mscn,cy,cx,rst,rct,theta,w):
    min_kurtosis = 100
    for time_index,time_length in enumerate([5,10,20,30]):
        for index,t in enumerate(theta):
            rsin_theta = rst[:,index]
            rcos_theta  =rct[:,index]
            x_sts,y_sts = cx+rcos_theta,cy+rsin_theta
            
            data=Y3d_mscn[time_index,:time_length,y_sts*w+x_sts].flatten() 
            # temporal filtering
            data_mu4 = np.mean((data-np.mean(data))**4)
            data_var = np.var(data)
            st_kurtosis = np.abs(data_mu4/(data_var**2+1e-4)-3)
            if(st_kurtosis<min_kurtosis):
                best_block = np.reshape(data,(len(x_sts),time_length))
...
def find_kurtosis_sts(grad_img_buffer,cy,cx,rst,rct,theta):
    ...
    grad3d_mscn = np.zeros((4,grad_img_buffer.shape[0],grad_img_buffer.shape[1]*grad_img_buffer.shape[2]))
    lgn_param_dict = {'5':0.5,'10':0.25,'20':0.15,'30':0.1}
    for index,time_length in enumerate([5,10,20,30]):
        ...
        grad3d_mscn[index,:,:] = scipy.ndimage.correlate1d(gradY3d_mscn,avg_window,1,mode='reflect')
    sts_grad= [find_kurtosis_slice(grad3d_mscn,\
...
def hdrchipqa_fromvid(filename,filename_out,width,height,framenos,bit_depth,color_space,step):
    ...
    for framenum in range(framenos): 
        ...
        if (i>=time_length): 
            ...
            # ST chips
            sts_grad = find_kurtosis_sts(grad_img_buffer,cy,cx,rst,rct,theta)
            dsts_grad = find_kurtosis_sts(graddown_img_buffer,dcy,dcx,rst,rct,theta)
            ...
            firstord_grad_feats = ChipQA.save_stats.estimateggdparam(np.concatenate([chip.flatten() for chip in sts_grad]))
            firstord_dgrad_feats = ChipQA.save_stats.estimateggdparam(np.concatenate([chip.flatten() for chip in dsts_grad]))
