...
def full_hdr_chipqa_forfile(i,filenames,results_folder,hdr,framenos_list=[]):
    ...
    # nonlinearity on PQ
    Y_pq_nl = Y_compute_lnl(prevY_pq,nl_method='exp',nl_param=4)
    Y_down_pq_nl =Y_compute_lnl(prevY_pq_down,nl_method='exp',nl_param=4)
    ...
    grad_img_buffer[i,:,:] =gradY_mscn_nl
    graddown_img_buffer[i,:,:]=dgradY_mscn_nl 
    i = i+1
...
def sts_fromvid(args):
    ...
    if(os.path.exists(outfolder)==False):
        os.mkdir(outfolder)
    Parallel(n_jobs=35)(delayed(full_hdr_chipqa_forfile)\
