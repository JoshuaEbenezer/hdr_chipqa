...
for i in range(rank, len(files), size):
    vname = files[i]
    results_folder = os.path.join(args.results_folder,str(args.time_length)+'_'+str(args.step))
    name = os.path.basename(vname)
    ...
    cmd = "python3 variable_length_chipqa.py --input_file {vname} --results_file {results_file} --bit_depth {bit_depth} --color_space BT709\
         --width 3840 --height 2160 --time_length {time_length} --step {step}".format(vname=vname,results_file=results_file,\
             bit_depth=bit_depth,time_length=args.time_length,step=args.step)
    print(cmd)
    os.system(cmd)
