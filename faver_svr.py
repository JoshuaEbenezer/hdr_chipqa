...
def results(all_preds,all_dmos):
    ...
    preds_srocc = np.nan_to_num(spearmanr(preds_fitted,all_dmos))
    preds_lcc = np.nan_to_num(pearsonr(preds_fitted,all_dmos))
    preds_rmse = np.nan_to_num(np.sqrt(np.mean((preds_fitted-all_dmos)**2)))
#    print('SROCC:')
#    print(preds_srocc[0])
#    print('LCC:')
#    print(preds_lcc[0])
...
scores_df = pd.read_csv('subject_MOS.csv')
print(len(scores_df))
...
#    feature_folder= "/home/ubuntu/bitstream_mode3_p1204_3/features/p1204_etri_features"
    ...
    feature_folder= './ythfr_features/entropy_features'
    train_names = []
    val_names = [] 
...
#        else:
        ...
        try:
            ...
            continue
#        feature1 = feat_file.flatten()
        score = scores[i]
        feature1 = np.average(feat_file['4_yt'])
        feature2 = np.average(feat_file['4_xt'])
        feature3 = np.max(feat_file['4_yt'])
        feature4 = np.max(feat_file['4_xt'])
        feature5 = np.min(feat_file['4_xt'])
        feature6 = np.min(feat_file['4_yt'])

        feature = np.stack((feature1,feature2,feature3,feature4,feature5,feature6),axis=0)
        feature = np.nan_to_num(feature,posinf=0,neginf=0,nan=0)
