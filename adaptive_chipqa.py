...
#@jit(nopython=True)
def find_kurtosis_slice(Y3d_mscn,cy,cx,rst_dict,rct_dict,theta,w):
    st_kurtosis = np.zeros((len(theta),))
    lgn_param_dict = {'5':0.5,'10':0.25,'20':0.15,'30':0.1}
    step_param_dict = {'5':30,'10':20,'20':10,'30':5}
    for time_length in [5,10,20,30]:
        mscn_data = Y3d_mscn[:time_length]
        ...
        grad3d_mscn = spatiotemporal_mscn(mscn_data,avg_window)
        for index,t in enumerate(theta):
            rsin_theta = rst_dict[str(time_length)][:,index]
            rcos_theta  =rct_dict[str(time_length)][:,index]
            x_sts,y_sts = cx+rcos_theta,cy+rsin_theta
            
            data=grad3d_mscn[:,y_sts*w+x_sts].flatten() 
            # temporal filtering
...
def hdrchipqa_fromvid(filename,filename_out,width,height,framenos,bit_depth,color_space,time_length,step):
    ...
    step_param_dict = {'5':30,'10':15,'20':8,'30':5}
    rct_list = []
    rst_list = []
    ...
    for key in step_param_dict.keys():
        step = step_param_dict[key]
    ...
    rct_list = np.asarray(rct_list)
    rst_list = np.asarray(rst_list)
    cy_list = np.asarray(cy_list)
    cx_list = np.asarray(cx_list)
    dcy_list = np.asarray(dcy_list)
    dcx_list = np.asarray(dcx_list)

    rst_dict = zip(step_param_dict.keys(),rst_list)
    rct_dict = zip(step_param_dict.keys(),rct_list)
    cy_dict = zip(step_param_dict.keys(),cy_list)
    cx_dict = zip(step_param_dict.keys(),cx_list)
    dcy_dict = zip(step_param_dict.keys(),dcy_list)
    dcx_dict = zip(step_param_dict.keys(),dcx_list)
    ...
    for framenum in range(framenos): 
        ...
        if (i>=time_length): 
            ...
            # ST chips
            sts_grad = find_kurtosis_sts(grad_img_buffer,cy_dict,cx_dict,rst_dict,rct_dict,theta)
            dsts_grad = find_kurtosis_sts(graddown_img_buffer,dcy_dict,dcx_dict,rst_dict,rct_dict,theta)
